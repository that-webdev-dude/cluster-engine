...
| 0013 | Render Recovery Driven By Execute | renderer/runtime/api | Accepted | RenderService.execute privately drives backend recovery without public recovery controls, authored hooks, or RenderFrameInput changes. |
| 0014 | Logical Render State Survives Backend Loss | renderer/runtime/api | Accepted | Logical resources and pipeline descriptors survive backend loss; native backend objects are destroyed or dropped and recreated lazily after recovery. |
| 0015 | WebGPU Recovery Uses Replacement Device | renderer/runtime | Accepted | WebGPU recovery acquires a replacement runtime generation and ignores stale loss completions from prior devices. |
| 0016 | Render Recovery Diagnostics Stay Existing | renderer/runtime/api | Accepted | Recovery diagnostics stay limited to existing state, lostBackend, selectedBackend, and fallback fields with no new public RenderView fields. |
